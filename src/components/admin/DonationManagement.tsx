...
import { Plus, Pencil, Trash2, Search, X, Heart, Target, Users, Phone, MessageCircle } from 'lucide-react';
...
export function DonationManagement() {
  ...
  const resetForm = () => {
    setFormData({
      title: '', description: '', imageURL: '', targetAmount: '',
      isUrgent: false, isFeatured: false, contactNumber: '', whatsappNumber: '',
    });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingCampaign) {
        // Preserve existing donation stats when editing
          ...
          <motion.div key={campaign.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
            ...
            <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex items-center gap-3 mt-1.5">
                    {campaign.contactNumber && (
                      <span className="flex items-center gap-1 text-[10px] text-emerald-600">
                        <Phone className="w-3 h-3" /> {campaign.contactNumber}
                      </span>
                    )}
                    {campaign.whatsappNumber && (
                      <span className="flex items-center gap-1 text-[10px] text-green-600">
                        <MessageCircle className="w-3 h-3" /> {campaign.whatsappNumber}
                      </span>
                    )}
                  </div>
                )}
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                <button onClick={() => openEdit(campaign)} className="p-1.5 rounded-lg hover:bg-emerald-50 dark:hover:bg-emerald-900/20">
                  <Pencil className="w-3.5 h-3.5 text-emerald-500" />
                </button>
                <button onClick={() => handleDelete(campaign.id)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/50" onClick={() => { setShowModal(false); resetForm(); }}>
          <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} onClick={(e) => e.stopPropagation()}
            ...
            <div className="space-y-4">
              ...
              <div className="border rounded-xl p-4 space-y-3" style={{ borderColor: 'var(--border-color)', background: 'var(--bg-primary)' }}>
                <p className="text-xs font-semibold" style={{ color: 'var(--text-muted)' }}>💳 Donation Contact Info</p>
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Account / Phone number (optional)"
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="WhatsApp number (e.g. +255700000000)"
                    className="w-full h-11 pl-10 pr-4 rounded-xl border text-sm outline-none focus:border-emerald-500"
                    style={{ background: 'var(--bg-secondary)', borderColor: 'var(--border-color)', color: 'var(--text-primary)' }}
                  />
                </div>
              ...
              <button
                onClick={handleSave}
                disabled={saving || uploading || !formData.title || !formData.description || !formData.targetAmount}
